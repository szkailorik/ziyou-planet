...
面向小学阶段儿童的本地优先识字训练 PWA。产品把“快速认识更多字”拆成可解释的诊断、短回合学习、跨日复测和家长报告，并把儿童学习界面与家长数据界面分开。
...
部署配置位于 `wrangler.jsonc`；`public/_headers` 提供安全与缓存头，`public/_redirects` 保证单页应用路由可回退到首页。

## 当前完成度

本仓库是可运行的 P0：家庭多儿童档案、3500 字底库、熟悉度扫描、字—词—句反馈、审核字客观读音复核、确定性掌握度、间隔复习、字册、家长报告、本地备份和 PWA 均已实现。逐册教材出处、完整 300 基本字的人工词语/多音字审核、自适应识字量区间及可选 AI 属于 P1，不能把自动生成的基础拼音当成已审教材事实。
