...
1. 按课标字表一、二分层，使用固定种子的伪随机顺序，避免 3500 字表按拼音排列造成连续取样偏差。
2. 首轮抽 36 字（字表一 24、字表二 12）快速定位；后续每轮追加 24 个未测代表字。
3. 使用各层“我会读”比例的 Wilson 95% 区间，按 2500/1000 的层级规模加权，只输出整百范围与可靠度，不输出伪精确整数。
4. 每轮结束即可休息；定位阶段保持单字自评，后续复习再切换到读音、词义/语境与跨日题型，避免作答线索互相污染。
5. 把“不确定、请教我、客观答错、到期”的字组成独立薄弱队列；扩大覆盖用于家长确需逐字盘点时分多日完成。

当前范围仍是儿童自报的“会读熟悉度”，不是稳定掌握。系统必须继续抽取自报会读的字进行读音、词义/语境与跨日复核；在完整审核题库建立前，不把范围包装成标准化常模测验结果。
