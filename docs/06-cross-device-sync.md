...
- 服务端生成约 80 bit 随机性的 `ZIYOU-XXXX-XXXX-XXXX-XXXX` 设备邀请码。
- 新设备需要同时输入设备邀请码和家长 PIN。
- 加入成功后，服务端下发随机设备会话 Cookie；Cookie 使用 `HttpOnly`、`Secure`、`SameSite=Strict`，前端 JavaScript 不可读取。
- D1 只保存邀请码的 SHA-256 摘要、PBKDF2 派生后的 PIN 摘要和设备令牌摘要，不保存明文。
- 一个家庭可以同时保留最多 12 个有效邀请码；生成新码不会覆盖旧码，也不会让已连接设备退出。
- 新邀请码 30 天内有效，每个最多加入 8 台设备；原有家庭同步码迁移为长期兼容邀请码，避免升级后已有家庭无法加入第三台设备。

设备邀请码是恢复凭证，应与 PIN 分开保管。产品不要求儿童真实姓名、学校、生日、手机号或邮箱。

## 数据表

### `families`

保存家庭 ID、兼容同步码摘要、PIN 盐与摘要、当前家庭设置 JSON、设置更新时间。兼容字段不再被新邀请码覆盖。

### `family_invites`

每个家庭可拥有多条邀请码记录，保存邀请码摘要、有效期、最多使用次数、已经使用次数和撤销状态。邀请码明文只在创建时返回一次。
...
验收至少覆盖：创建家庭、写入事件、第二和第三设备使用同一码加入、生成新邀请码后旧邀请码仍可加入、新设备读取同一事件、重复上传去重、错误 PIN 拒绝、退出后会话失效、断网时本地学习不受影响。
