...
中文识字是主任务。英文和字族都属于答后增强层：先让孩子只看汉字独立回忆；点“教教我”后，在当前屏立即出现读音、简明字义、中文词句和英文语义桥。字族与文化彩蛋不放进这个快速教学卡，避免分散注意力。这些信息不会泄露作答前的答案，也不计入中文掌握度。
...
为避免点“教教我”后再等待大模型，前两轮分层定位会遇到的 60 个代表字已预生成并固化为本地数据。每条都要通过结构校验：至少两个含目标字的词、一个含目标字的生活句、一条绑定具体中文词的英文桥；页面点击时不会调用实时模型。
